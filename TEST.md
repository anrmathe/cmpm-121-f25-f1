...
This project includes automated tests for the 2D and 3D Sudoku logic. Tests are written using a custom test framework (`minitest.lua`) and verify the core functionality of the Sudoku solvers and validators.
...
- **`tests/minitest.lua`** - Custom test framework providing `describe`, `it`, `assert_*` functions
- **`tests/spec_helper.lua`** - Test utilities and setup

### Test Runner

- **`run_tests.lua`** - Main test runner that loads all test files and executes them
...
#### 2. **isBoardComplete only returns true for a valid full board**
   ...
   - **Test Cases:**
     - Returns `true` for a fully solved 9×9 board with all values 1-9 in each row/column/box
